...
const ProductBanner = ({ itemsPerPageFromBanner, totalProducts, onSortChange, onViewChange }) => {
    ...
    <div className="w-full flex flex-col md:flex-row md:items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <span
          ...
          } w-8 h-8 text-lg flex items-center justify-center cursor-pointer gridView transition duration-300`}
        >
          <BsGridFill />
        </span>
        <span
          className={`${
            listViewActive
              ? "bg-primeColor text-white"
              : "border-[1px] border-gray-300 text-[#737373]"
          } w-8 h-8 text-base flex items-center justify-center cursor-pointer listView transition duration-300`}
        >
          <ImList />
        </span>
